...
function ocdla_default_preprocess_html()
{
	global $request;
		...
		$settings = new Script( array( 'type' => SCRIPT_LOCAL_FILE, 'path' => '/sites/all/themes/ocdla/js/settings.js' ));
		...
		clickpdx_add_js( $settings, THEME_SCRIPT_REGION_FOOTER );
		clickpdx_add_js( $jquery, THEME_SCRIPT_REGION_HEADER );
		clickpdx_add_js( $js_menus, THEME_SCRIPT_REGION_HEADER );
		clickpdx_add_js( $ajax_globals, THEME_SCRIPT_REGION_FOOTER );
		clickpdx_add_js( $ocdla, THEME_SCRIPT_REGION_FOOTER );		

		if( strpos( drupal_get_path_alias(),'edit')!==FALSE)
		{
			clickpdx_add_js( $ckeditor, THEME_SCRIPT_REGION_HEADER );
			clickpdx_add_js( $inline_editing, THEME_SCRIPT_REGION_HEADER );
		}
