define([],function(){
	...
	function validateExpirationData(monthYear){
	...
	function validateAmount(data){
		var validators = {
			amount: function(data){
				return (typeof data == 'undefined' || data == '' || parseInt(data) < 1) ? 0.00 : data }
			}
		};	
	}
	...
	function validateBilling(data){
		...
		ccNum: function(data){
			if(false && data.length != 16) {
				throw new Error('Credit card number must be 16 digits.');
			}
			return data;
		},
		ccExp: function(data){ return parseCcExp(data); },
