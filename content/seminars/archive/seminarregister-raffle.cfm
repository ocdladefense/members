...
<cfif not isdefined("URL.ID")>
	<cflocation url="https://www.ocdla.org">
</cfif>
...
function goForum(){
	popMenu = window.open('http://www.ocdla.org/PopUpLogin.cfm?page=FO', 'UserLogin', 'top=200,left=200,width=330,height=400,toolbar=no,menubar=no,scrollbars=no,location=no,directories=no,status=no,resizable=yes');
	return true;
}
...
<tr>
	<td>
		<a href="#" onClick="JavaScript:window.open('http://www.ocdla.org/PopUpLogin.cfm?page=CAT', 'UserLogin', 'top=200,left=200,width=330,height=400,toolbar=no,menubar=no,scrollbars=no,location=no,directories=no,status=no,resizable=yes');" 
		...
		<a href="#" onClick="JavaScript:window.open('http://www.ocdla.org/PopUpLogin.cfm?page=LD', 'UserLogin', 'top=200,left=200,width=330,height=400,toolbar=no,menubar=no,scrollbars=no,location=no,directories=no,status=no,resizable=yes');" 
		...
		<a href="#" onClick="JavaScript:window.open('http://www.ocdla.org/PopUpLogin.cfm?page=DP', 'UserLogin', 'top=200,left=200,width=330,height=400,toolbar=no,menubar=no,scrollbars=no,location=no,directories=no,status=no,resizable=yes');" 
		...
		<a href="#" onClick="JavaScript:window.open('http://www.ocdla.org/PopUpLogin.cfm?page=FO', 'UserLogin', 'top=200,left=200,width=330,height=400,toolbar=no,menubar=no,scrollbars=no,location=no,directories=no,status=no,resizable=yes');" 
...
<tr>
	...
	&nbsp;<br>&nbsp;<br>&nbsp;<br><a href=".http://www.ocdla.org/RequestUP.cfm"><img src="/images/mainnav_request2.gif" width=130 height=20 border=0 alt=""></a>
	</td>
</tr>      </table></td>
...
<form name="seminarform" method="post" action="../cart/seminaradd.cfm" onSubmit="return submitIt(this)">
	<cfoutput><input type="hidden" name="ID" value="#getSeminar.ID#"></cfoutput>
...
<!---			    	<input onMouseOver="alert('If you are an OCDLA member, log-in at left \nto take advantage of member discounts. \n\nIf you are not a member, JOIN NOW.');document.forms.seminarform.seminarprice[#SPIndex#].blur();" type="radio" name="seminarprice" value="#getSeminar.MemberNonLawyerStandard#"><cfset SPIndex = SPIndex +1> --->
		...
		<cfif getSeminar.EventIDList neq ''>
			...
			<cfquery name="getEvents" datasource="OCDLA_Data">
				...
				FROM catalog
				WHERE Category = 'Events'
				AND ID IN(#PreserveSingleQuotes(getSeminar.EventIDList)#)
				ORDER BY OrderPriority ASC;
			</cfquery>
			<cfset EventIndex = 0>
			<cfoutput query="getEvents">
				...
				<tr> 
				  <td colspan="3" class="text" height="2"> 
				    <input type="checkbox" name="Event_#EventIndex#" value="#getEvents.ID#">
		...
		<cfif getSeminar.RaffleIDList neq ''>
			<tr> 
			  <td colspan="3" class="text" bgcolor="#5A7882"><b><font color="#FFFFFF">Raffle Tickets</font></b></td>
			</tr>

			<cfquery name="getRaffle" datasource="OCDLA_Data">
				SELECT *
				FROM catalog
				WHERE Category = 'Raffle Tickets'
				AND ID IN(#PreserveSingleQuotes(getSeminar.RaffleIDList)#)
				ORDER BY OrderPriority ASC;
			</cfquery>
			<cfset RaffleIndex = 0>
			<cfoutput query="getRaffle">
				<cfset RaffleIndex = RaffleIndex + 1>
				<tr> 
				  <td colspan="3" class="text" height="2"> 
				    <input type="checkbox" name="Raffle_#RaffleIndex#" value="#getRaffle.ID#">&nbsp;#getRaffle.Item#<br>
				    <input type="text" name="RaffleTickets_#RaffleIndex#" size="3">&nbsp;ticket(s).
                    #getRaffle.Description#
					</td>
				</tr>
			</cfoutput>
		</cfif>
		...
		<cfoutput query="getMemberships">
			<tr> 
			  <td colspan="3" height="31" bgcolor="EEEEEE" class="text"> 
			    <input type="radio" name="membership" value="#getMemberships.ID#">
				  ...
				  <td width="29%" align="center" bgcolor="EEEEEE" class="text"> 
				    <cfif (isdefined("session.MBID") and session.MBID neq "" and NoNonMembers contains "|#ID#|") or not NoNonMembers contains "|#ID#|">
					    <cfif isdefined("session.MBID") and session.MBID neq "">
							<input type="checkbox" name="Material_#MaterialIndex#" value="#getMaterials.ID#"><cfset MPIndex = MPIndex +1>
							<cfif #getMaterials.RegularPrice# LT 0> Subtract </cfif>
					    	#DollarFormat(getMaterials.RegularPrice)#
						<cfelse>
<!---						<input onMouseOver="alert('If you are an OCDLA member, log-in at left \nto take advantage of member discounts. \n\nIf you are not a member, JOIN NOW.');document.forms.seminarform.Material_#MaterialIndex#[#MPIndex#].blur();" type="radio" name="Material_#MaterialIndex#" value="#getMaterials.ID#"><cfset MPIndex = MPIndex +1> --->
							<input type="checkbox" name="Material_#MaterialIndex#" value="#getMaterials.ID#">
							<cfif #getMaterials.RegularPrice# LT 0> Subtract </cfif>
					    	#DollarFormat(getMaterials.RegularPrice)# 
						</cfif>
				    <cfelse>
<!---				    <input onMouseOver="alert('If you are an OCDLA member, log-in at left \nto take advantage of member discounts. \n\nIf you are not a member, JOIN NOW.');document.forms.seminarform.Material_#MaterialIndex#.blur();" type="checkbox" name="Material_#MaterialIndex#" value="#getMaterials.ID#"> --->
					    <input type="checkbox" name="Material_#MaterialIndex#" value="#getMaterials.ID#">
						<cfif #getMaterials.RegularPrice# LT 0> Subtract </cfif>
					    #DollarFormat(getMaterials.RegularPrice)# 
				    </cfif>
				  ...
				  <cfset MaterialIndex = MaterialIndex+1>
				  <td width="29%" align="center" bgcolor="EEEEEE" class="text"> 
				    <cfif getMaterials.Price neq 0><input type="checkbox" name="Material_#MaterialIndex#" value="#getMaterials.ID#"><cfset MPIndex = MPIndex +1>
				    <cfif #getMaterials.RegularPrice# LT 0> Subtract </cfif>
...
<div class="basemenu">
  <div align="center"><a href="/index.html" class="basemenu">Home</a> | <a href="../seminars.html" class="basemenu">Seminars</a> 
    ...
    <a href="http://www.ocdla.org/contact.html" class="basemenu">Contact</a> | <a href="../cart/index.cfm">Store</a> | 
    <a href="http://www.ocdla.org/sitemap.html" class="basemenu">Site Map</a><br>
    <br>
