...
<cfquery name="getSeminar" datasource="OCDLA_test_Data">
	SELECT *
	FROM catalog
	WHERE ID = '#URL.ID#';
</cfquery>
...
<!---			    	<input onMouseOver="alert('If you are an OCDLA member, log-in at left \nto take advantage of member discounts. \n\nIf you are not a member, JOIN NOW.');document.forms.seminarform.seminarprice[#SPIndex#].blur();" type="radio" name="seminarprice" value="#getSeminar.MemberNonLawyerStandard#"><cfset SPIndex = SPIndex +1> --->
		...
		<cfquery name="getNoNonMembers" datasource="OCDLA_test_Data">
			SELECT ID
			FROM catalog
		...
		<cfif getSeminar.MaterialIDList neq '' AND getSeminar.OptionDesc5 EQ 'temp'>
			...
			<cfquery name="getMaterials" datasource="OCDLA_test_Data">
				SELECT *
				FROM catalog
		...
		<cfif getSeminar.EventIDList neq ''>
			...
			--->
			<cfquery name="getEvents" datasource="OCDLA_test_Data">
				SELECT *
				FROM catalog
				WHERE Category = 'Events'
		...
		<cfquery name="getMemberships" datasource="OCDLA_test_Data">
			SELECT *
			FROM catalog
		...
		<cfif getSeminar.MaterialIDList neq '' AND getSeminar.OptionDesc5 NEQ 'temp'>
			...
			<cfquery name="getMaterials" datasource="OCDLA_test_Data">	<!--- I created same name above and possibly cause some problem in the future --->
				SELECT *
				FROM catalog
